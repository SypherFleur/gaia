...
### Fifth tier — production hardening, 2026-08-15

Suite after this tier: **430 tests, 0 failures, 8 opt-in skips**, now enforced by CI. Two defects in this tier were found during hardening rather than in the original audit, and both were latent rather than visible in tests.

- **Latching circuit breaker (new S1-class find).** `ProviderHealthMonitor` marked a provider `UNAVAILABLE` after two consecutive failures, and `ToolGateway.execute` denies `UNAVAILABLE` providers at line 142 — which returns *before* line 206, the only call site of `record_success`. A provider that tripped the breaker therefore could never recover for the life of the process; two transient network blips permanently disabled NWS, GBIF, or any other live provider until restart. Fixed with a half-open probe: after a cooldown (60 s, doubling per open cycle, capped at 900 s) exactly one request is admitted to test the provider, and its outcome closes or re-opens the circuit. `snapshot()` exposes `seconds_until_retry` for diagnostics.
- **No retry anywhere.** A single transient failure failed the whole request. `packages/providers/http_retry.py` adds bounded retry with exponential backoff and full jitter, retrying only genuinely transient conditions (connection errors, timeouts, 408/425/429/5xx) and re-raising non-retryable `HTTPError` immediately so adapters keep mapping 404/400/401 to precise statuses. Wired into Census, SSURGO, USGS Water, and Genesys.
- **S3-3 closed.** `packages/provenance/identifiers.py` scans every string in a payload — narrative prose included — for DOI/PMID/PMCID patterns. GuidancePlans reject any bibliographic identifier outright, since GAIA attaches source records itself and never hands the model a citation to reuse; research syntheses reject any identifier not backed by a retrieved work id, with normalization so `doi:X`, `https://doi.org/X`, and bare `X` compare equal. Verified not to flag ordinary prose containing decimals, dates, or version numbers.
- **Latent circular import fixed.** `packages.cost.firewall` imported `packages.providers.registry` at module level while the latter imports `packages.cost.policy`, so any process importing `packages.providers` first crashed. No existing test hit that order; the new resilience tests did. `BillingClass` is now imported at call time with the reason documented in place.
- **CI added** (`.github/workflows/ci.yml`): the offline suite plus constitution check across Linux/macOS/Windows on Python 3.12 and 3.13 — the matrix exists because the command surface silently regressed to Windows-only once. Two guardrail jobs assert the financial constitution still disables automatic paid usage and caps the reserve at $20, and that every live smoke stays behind a `GAIA_RUN_*_SMOKE` guard so CI can never depend on a live provider.
